...
define('FOLDER_PLAYERNAMES', BASE_FOLDER . '/admin/config/names/import/players');

$mainTitle = $i18n->getMessage("importplayers_navlabel");
...
$player_files = "";
$it = new FilesystemIterator(FOLDER_PLAYERNAMES);
foreach ($it as $fileinfo) {
    $player_files .= $fileinfo->getFilename() .",";
}
$filelist = substr($player_files, 0, -1);

if (!$show) {

  ?>
  
  <p><?php echo $i18n->getMessage("importplayers_intro"); ?></p>
...
elseif ($show == "generate") {
  ...
  if (isset($err)) {
	...
	DataImportService::generatePlayers($website, $db, $_POST['league'], $_POST['importfile']);
