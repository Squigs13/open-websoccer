...
class LendPlayerController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($teamSize <= $this->_websoccer->getConfig("transfermarket_min_teamsize")) {
			throw new Exception($this->_i18n->getMessage("loan_err_teamsize_too_small", $teamSize));
		}
		
		// remaining contract is too short
		$minBidBoundary = round($player["player_marketvalue"] / 2);
		if ($player["player_contract_matches"] <= $this->_websoccer->getConfig("loan_matches_min")) {
			throw new Exception($this->_i18n->getMessage("loan_err_contract_too_short"));
		}
